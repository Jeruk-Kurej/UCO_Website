                        ...
                        @foreach($photos as $photo)
                            ...
                            <div x-data="{
                                ...
                                <div class="relative group">
                                    ...
                                    {{-- Caption overlay --}}
                                    @if($photo->caption)
                                        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent text-white text-xs px-3 py-2">
                                            {{ $photo->caption }}
                                        </div>
                                    @endif
                                    ...
                                    @if(auth()->id() === $product->business->user_id || auth()->user()->isAdmin())
                                        <div x-show="showUpdate"
                                            ...
                                            <form method="POST"
                                                    ...
                                                    <div class="flex items-center gap-3 p-3 bg-white border border-gray-200/80 rounded-xl">
                                                        ...
                                                        <div class="flex flex-col items-center gap-1">
                                                            ...
                                                            <div class="w-16 h-16 rounded-lg bg-white border border-gray-200 flex items-center justify-center overflow-hidden shadow-sm">
                                                                <img :src="currentImage" alt="{{ $photo->caption }}" class="w-full h-full object-cover">
                                                            </div>
                                                        </div>
                                                ...
                                                {{-- Caption field --}}
                                                <div>
                                                    <label for="caption_{{ $photo->id }}" class="block text-xs font-medium text-gray-600 mb-1">
                                                        Caption <span class="text-gray-400">(Opsional)</span>
                                                    </label>
                                                    <input type="text"
                                                           name="caption"
                                                           id="caption_{{ $photo->id }}"
                                                           value="{{ old('caption', $photo->caption) }}"
                                                           class="block w-full px-3 py-2 text-sm border border-gray-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 transition"
                                ...
                                {{-- Caption (below photo, when no update panel) --}}
                                <div class="px-4 py-3" x-show="!showUpdate">
                                    @if($photo->caption)
                                        <p class="text-sm text-gray-700 font-medium truncate">{{ $photo->caption }}</p>
                                    @else
                                        <p class="text-sm text-gray-400 italic">Tidak ada caption</p>
                                    @endif
                                    <p class="text-xs text-gray-400 mt-0.5">{{ $photo->created_at->diffForHumans() }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
