<x-app-layout>
    <div class="max-w-5xl mx-auto">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
            <a href="{{ route('business-types.show', $businessType) }}" 
            ...
            <div class="flex-1">
                <h1 class="text-2xl font-bold text-gray-900">Edit Business Type</h1>
        ...
        <div class="bg-white shadow-sm sm:rounded-xl">
            <div class="p-6">
                <form method="POST" action="{{ route('business-types.update', $businessType) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  id="description" 
                                  rows="4" 
                                  class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-soft-gray-900 focus:ring-soft-gray-900 sm:text-sm @error('description') border-red-500 @enderror">{{ old('description', $businessType->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-sm text-blue-800">
                                    This business type is currently used by <strong>{{ $businessType->businesses->count() }}</strong> business(es).
                                </p>
                                @if($businessType->businesses->count() > 0)
                                    <p class="mt-2 text-xs text-blue-700">
                                        Deleting this type is not allowed while businesses are using it.
                                    </p>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                        <a href="{{ route('business-types.show', $businessType) }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 hover:text-gray-900 rounded-xl transition duration-150">
                            Cancel
                        </a>
                        <div class="flex items-center gap-3">
                            @if($businessType->businesses->count() === 0)
                                <button type="button" 
                                        onclick="if(confirm('Are you sure you want to delete this business type?')) document.getElementById('delete-form').submit();"
                            ...
                            @else
                                <button type="button" 
                                        disabled
                                        class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-400 rounded-xl cursor-not-allowed"
                ...
                @if($businessType->businesses->count() === 0)
                    <form id="delete-form" action="{{ route('business-types.destroy', $businessType) }}" method="POST" class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
                @endif
