<x-app-layout>
    <div class="space-y-20 pb-8">
        {{-- Hero --}}
        <section class="relative overflow-hidden rounded-3xl border border-uco-orange-100 bg-white px-6 py-12 shadow-sm md:px-10 md:py-16">
                        ...
                        <a href="{{ route('businesses.show', $business) }}"
                           class="group overflow-hidden rounded-2xl border border-soft-gray-100 bg-white p-3 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                            <div class="h-28 overflow-hidden rounded-xl bg-soft-gray-100">
                                ...
                                @else
                                    <div class="flex h-full w-full items-center justify-center bg-gradient-to-br from-uco-orange-100 to-uco-yellow-100">
                            ...
                            <div class="pt-3">
                                ...
                                <p class="line-clamp-1 text-xs text-soft-gray-500" title="{{ $business->businessType->name ?? 'Business' }}">{{ $business->display_category ?? ($business->businessType->name ?? 'Business') }}</p>
                            </div>
                        </a>
                    @endforeach
                </div>
                    ...
                    <a href="{{ route('businesses.show', $business) }}"
                       ...
                       class="group overflow-hidden rounded-2xl border border-soft-gray-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:border-uco-orange-300 hover:shadow-xl reveal-on-scroll flex flex-col">
                        <div class="relative h-48 overflow-hidden sm:h-52">
                            ...
                            @endif
                            <div class="absolute inset-0 bg-gradient-to-t from-soft-gray-900/70 via-soft-gray-900/10 to-transparent"></div>
                            <div class="absolute left-4 top-4">
                                <span class="rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-soft-gray-800 backdrop-blur" title="{{ $business->businessType->name ?? 'Business' }}">
                        ...
                        <div class="flex flex-1 flex-col justify-between space-y-4 p-5">
                            ...
                            <div class="flex items-center justify-between rounded-xl bg-soft-gray-50 px-3 py-2">
                                <div class="flex items-center gap-2">
                                    ...
                                    @else
                                        <div class="flex h-9 w-9 items-center justify-center rounded-full bg-uco-orange-100 text-sm font-bold text-uco-orange-700">
                                            {{ strtoupper(substr($business->user->name ?? 'U', 0, 1)) }}
                                        </div>
                                    @endif
                                    <div>
                                        <p class="text-sm font-semibold text-soft-gray-900">{{ $business->user->name ?? 'UCO Member' }}</p>
                                        <p class="text-xs text-soft-gray-500">{{ ucfirst($business->business_mode) }} business</p>
                                    </div>
                                </div>
        ...
        {{-- UCO Support Journey --}}
        <section class="rounded-3xl border border-uco-yellow-100 bg-gradient-to-br from-uco-yellow-50 to-white p-6 md:p-8">
            <div class="mb-8 reveal-on-scroll">
                <h2 class="text-3xl font-bold text-soft-gray-900">How UCO Supports Entrepreneurial Growth</h2>
                <p class="mt-2 text-sm text-soft-gray-600">From first idea to scaling up, we support founders through each phase.</p>
            </div>
        ...
        @if($testimonies->count() > 0)
            <section class="space-y-6"
                     x-data="{ index: 0, total: {{ $testimonies->count() }}, timer: null, next(){ this.index = (this.index + 1) % this.total }, prev(){ this.index = (this.index - 1 + this.total) % this.total }, start(){ this.timer = setInterval(() => this.next(), 4500) }, stop(){ if (this.timer) clearInterval(this.timer) } }"
                     x-init="start()"
                     @mouseenter="stop()"
                     @mouseleave="start()">
                <div class="flex items-end justify-between reveal-on-scroll">
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="prev()" class="rounded-full border border-soft-gray-300 p-2 text-soft-gray-600 transition hover:border-uco-orange-300 hover:text-uco-orange-600">
                            <i class="bi bi-chevron-left"></i>
                        </button>
                        <button @click="next()" class="rounded-full border border-soft-gray-300 p-2 text-soft-gray-600 transition hover:border-uco-orange-300 hover:text-uco-orange-600">
                            <i class="bi bi-chevron-right"></i>
                        </button>
                    </div>
                </div>

                <div class="relative overflow-hidden rounded-3xl border border-soft-gray-200 bg-white p-6 shadow-sm md:p-8 reveal-on-scroll" style="transition-delay: 100ms;">
        ...
        <section class="space-y-6">
            ...
            <div class="relative overflow-hidden rounded-2xl border border-soft-gray-200 bg-white py-5 reveal-on-scroll" style="transition-delay: 100ms;">
        ...
        {{-- WhatsApp CTA --}}
        <section class="overflow-hidden rounded-3xl border border-uco-orange-200 bg-gradient-to-r from-uco-orange-50 via-white to-uco-yellow-50 p-6 md:p-8 reveal-on-scroll">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-soft-gray-900 md:text-3xl">Have a UCO business to showcase?</h2>
                    <p class="mt-2 text-sm text-soft-gray-600">Reach out and become part of our growing student & alumni business directory.</p>
                </div>
